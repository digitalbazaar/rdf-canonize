...
- Allow `canonicalIdMap` to be passed to `canonize` to be used by the canonical
  issuer identifier. This allows the bnode identifier mapping generated by the
  canonicalization algorithm to be exported to the caller. This feature is
  particularly useful when the resulting bnode labels need to be changed for use cases such as selective disclosure.

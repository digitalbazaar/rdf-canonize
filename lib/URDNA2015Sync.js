...
module.exports = class URDNA2015Sync {
  ...
  main(dataset) {
    ...
    for(const quad of dataset) {
      ...
      this.forEachComponent(quad, component => {
        ...
        if(info) {
          ...
          this.blankNodeInfo.set(id, {quads: [quad]});
        }
      });
    }
    ...
    // identifiers:
    // TODO: use Map and store Uint8Array/Buffers instead of hex, sort
    // works the same way with hex vs. integer representation
    ...
    for(const idList of nonUnique) {
      ...
      // TODO: use `_bufferCompare` that will use `.compare` for Buffer
      // and that will loop for non-Buffer Uint8Array
      ...
      for(const result of hashPathList) {
        ...
        const existing = [...result.issuer.existing.keys()];
  ...
  hashFirstDegreeQuads(id) {
    ...
    for(const quad of quads) {
      ...
      this.forEachComponent(quad, (component, key) => {
        ...
        copy[key] = this.modifyFirstDegreeComponent(id, component, key);
      });
  ...
  hashRelatedBlankNode(related, quad, issuer, position) {
    ...
    if(this.canonicalIssuer.hasId(related)) {
      ...
      id = this.hashFirstDegreeQuads(related);
    }
  ...
  hashNDegreeQuads(id, issuer) {
    ...
    for(const hash of hashes) {
      ...
      // TODO: rename to `Permuter`
      const permutator = new Permutator(hashToRelated.get(hash));
    ...
    // through the hash algorithm.
    return {hash: _toHex(md.digest()), issuer};
  }
  ...
  modifyFirstDegreeComponent(id, component) {
    ...
    /* Note: A mistake in the URDN2015 spec that made its way into
...
function _toHex(hash) {
  // TODO: determine how to optimize away `hex`

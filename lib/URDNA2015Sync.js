...
const POSITIONS = {'subject': 's', 'object': 'o', 'name': 'g'};
...
module.exports = class URDNA2015Sync {
  ...
  main(dataset, callback) {
    ...
    while(simple) {
      ...
      for(let id in nonNormalized) {
        ...
        // Quads algorithm.
        const hash = self.hashFirstDegreeQuadsSync(id);
    ...
    for(let i = 0; i < hashes.length; ++i) {
      ...
      for(let j = 0; j < idList.length; ++j) {
        ...
        const result = self.hashNDegreeQuadsSync(id, issuer);
  ...
  hashFirstDegreeQuads(id) {
    ...
    for(let i = 0; i < _nquads.length; ++i) {
      md.update(_nquads[i]);
    }
    info.hash = md.digest();
    return info.hash;

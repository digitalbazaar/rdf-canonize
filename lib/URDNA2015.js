...
module.exports = class URDNA2015 {
  constructor(
    {maxDeepIterations = Infinity, useHmac = false, hmacKey = null} = {}
  ) {
    this.name = 'URDNA2015';
    ...
    if(this.hmacKey && !this.useHmac) {
      process.emitWarning(
        'Hmac key is ignored. Set useHmac = true, to use Hmac');
    }
  ...
  async hashFirstDegreeQuads(id) {
    ...
    const digest = new MessageDigest.getMessageDigest(
      {
  ...
  async hashRelatedBlankNode(related, quad, issuer, position) {
    ...
    // Note: We use a hash (we dont use Hmac here) object instead.
    const md = new MessageDigest.getMessageDigest(
      {
        hashAlgorithm: this.hashAlgorithm
      }
    );
  ...
  async hashNDegreeQuads(id, issuer) {
    ...
    // Note: We created a hash object `digest` above instead.
    ...
    // through the hash algorithm.
    return {hash: await digest.digest(), issuer};
  }

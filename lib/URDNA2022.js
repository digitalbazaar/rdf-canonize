...
module.exports = class URDNA2022 {
  constructor(options) {
    this.name = 'URDNA2022';
  ...
  async hashFirstDegreeQuads(id) {
    ...
    const hmac = new MessageDigest.Hmac(this.hashAlgorithm, this.hmacKey);
    for(const nquad of nquads) {
      hmac.update(nquad);
    }
    info.hash = await hmac.digest();
    return info.hash;
  }
  ...
  async hashRelatedBlankNode(related, quad, issuer, position) {
    ...
    const md = new MessageDigest.Hash(this.hashAlgorithm);
    md.update(position);
  ...
  async hashNDegreeQuads(id, issuer) {
    ...
    // Note: We created a HMAC object `hmac` above instead.
    ...
    // through the hash algorithm.
    return {hash: await hmac.digest(), issuer};
  }

...
module.exports = class URDNA2018Sync {
  ...
  computeOrbitHash({info, orbit, issuer = this.canonicalIssuer}) {
    if(orbit === 1) {
      info.state = {
        ...
        nextOrbiting: null,
        orbitHashes: [],
        orbitHashComplete: false
      };
    } else {
      if(info.state.orbitHashComplete) {
        // done, can't modify hash further
        return info.state.orbitHashes[info.state.orbitHashes.length - 1];
      }
    ...
    if(orbitQuads in this.hashCache) {
      ...
      orbitHash = this.hashCache[orbitQuads] = orbitQuads;
    }
    info.state.orbitHashes[orbit] = orbitHash;

    // compute the next orbiting set
    let nextSet = info.state.nextOrbiting || info.state.orbiting;
    info.state.nextOrbiting = new Set();
      ...
      for(const quad of quads) {
        this.forEachComponent(quad, component => {
          ...
          if(component.termType === 'BlankNode' &&
            ...
            if(!info.state.orbiting.has(candidate)) {
              info.state.orbiting.add(candidate);
              info.state.nextOrbiting.add(candidate);
            }
          }
        });
      }
    ...
    if(info.state.nextOrbiting.size === 0) {
      info.state.orbitHashComplete = true;
    }

    return info.state.orbitHashes[orbit];
  }
  ...
  modifyFirstDegreeComponent({info, component, orbit, issuer}) {
    ...
    if(orbit === 1) {
      info.state.orbiting.add(componentInfo);
    } else {
      component.value += componentInfo.state.orbitHashes[orbit - 1];
    }
    return component;
  }
